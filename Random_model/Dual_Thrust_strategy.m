%% 简介：Dual Thrust
%  入场条件：
%    当价格向上突破上轨时，如果当时持有空仓，则先平仓，再开多仓；如果没有仓位，则直接开多仓； 上轨=Open + Ks*max(HH-LC,HC-LL)
%    当价格向下突破下轨时，如果当时持有多仓，泽县平川，再开空仓；如果没有仓位，则直接开空仓； 下轨=Open - Kx*min(HH-LC,HC-LL)
...
HC=zeros(length(data),1); 
LC=zeros(length(data),1); 
LL=zeros(length(data),1);      
Range=zeros(length(data),1);            
...
Cash=repmat(1e7,length(data),1);               %可用资金,初始资金为100W
DynamicEquity=repmat(1e7,length(data),1);      %动态权益,初始资金为100W
StaticEquity=repmat(1e7,length(data),1);       %静态权益,初始资金为100W

%% --计算range和上下轨--
for i=1:Thrust_Length
  HH(i)=max(High(1:i));
  HC(i)=max(Close(1:i));
  LC(i)=min(Close(1:i));
  LL(i)=min(Low(1:i));
...
for i=Thrust_Length+1:length(data)
  HH(i)=max(High(i-Thrust_Length:i));
  HC(i)=max(Close(i-Thrust_Length:i));
  LC(i)=min(Close(i-Thrust_Length:i));
  LL(i)=min(Low(i-Thrust_Length:i));
end
Range = max(HH-LC,HC-LL);
UpperLine=Open+Ks*Range; 
LowerLine=Open-Kx*Range;
...
for i=Thrust_Length:length(data)
    ...
    %开多头
    if MarketPosition~=1  &&  High(i)>=UpperLine(i-1)   
        %平空开多
        if MarketPosition==-1   
            MarketPosition=1;
...
% end
sheetName = '交易汇总';
% xlswrite('测试报告.xlsx_交易汇总',TradeSum,sheetName);
%% 输出交易记录
...
%% 交易记录整体写入Excel
sheetName = '交易记录';
% xlswrite(filePath,TradeRec,sheetName);
%% 输出资产变化
...
%% 资产变化整体写入Excel
sheetName = '资产变化';
% xlswrite(filePath,TradeMoney,sheetName);
%% --图表分析--

%画出布林带(部分)
scrsz = get(0,'ScreenSize');
figure('Position',[scrsz(3)*1/4 scrsz(4)*1/6 scrsz(3)*4/5 scrsz(4)]*3/4);
candle(High(end-150:end),Low(end-150:end),Open(end-150:end),Close(end-150:end),'b');
hold on;
plot(UpperLine(end-150:end),'r');
plot(LowerLine(end-150:end),'g');
title('Dual Thrust 带(部分)');
saveas(gcf,'1.Dual Thrust 带.png');
close all;
...
scrsz = get(0,'ScreenSize');
figure('Position',[scrsz(3)*1/4 scrsz(4)*1/6 scrsz(3)*4/5 scrsz(4)]*3/4);
plot(Date,CumRateOfReturn,'r','LineWidth',2);
hold on;
% area(Date,DynamicEquity,'FaceColor','g');
datetick('x',29);
axis([Date(1) Date(end) min(CumRateOfReturn) max(CumRateOfReturn)]);
xlabel('时间');
ylabel('动态权益(元)');
title('权益曲线图');
hold off;
