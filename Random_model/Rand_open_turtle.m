...
for i=2:length(Close)
    ...
    if i==sign(find(sign>=barry,1))
        ...
        if MarketPosition~=0
%             MarketPosition=0;
            ShortMargin(i+a)=0;   %平空后空头保证金为0了
            LongMargin(i+a)=0;
            MyEntryPrice(i+a)=Close(i+a);
            MyEntryPrice(i+a)=MyEntryPrice(i+a)+Slip*MinMove*PriceScale;%建仓价格(也是平空仓的价格)
            ClosePosNum=ClosePosNum+1;
            ClosePosPrice(ClosePosNum)=MyEntryPrice(i+a);%记录平仓价格
            %       CloseDate(ClosePosNum)=Date(i+a);%记录平仓时间
            barry=i+a;
        end
    end
end
DynamicEquity10=DynamicEquity;
%
% %% -绩效计算--
%
...
% plot(1:length(DynamicEquity),DynamicEquity,'r','LineWidth',2);
% hold on;
