%% 简介：神奇 TD策略
%  策略简介： 详见广发证券研报??基于 GFTD 的期指日内程序化交易策略
clear
clc
...
%策略变量
Ud=zeros(length(data),1);
SumUd=zeros(length(data),1);
StopLoss=zeros(length(data),1);
...
%% --计算Ud,SumUdNum--
for i=n1+1:length(data)
    if Close(i)>Close(i-n1)
        Ud(i)=1;
    elseif Close(i)<Close(i-n1)
        Ud(i)=-1;
    else
        Ud(i)=0;
    end
...
for i=n1:length(data)
    ...
    %开多头
    if MarketPosition~=1 && SumUd(i)==-n2
        %平空开多
        BuyCount=0;
        for j=i+1:length(data)
            if Close(j)>=High(j-2) && High(j)>High(j-1) && Close(j)>Close(j-1)
              BuyCount=BuyCount+1;
            end
            if SumUd(j)==-n2 && BuyCount<n3
              BuyCount=0;
            end
            StopLoss(j)=
            if MarketPosition==-1 && BuyCount==n3
                MarketPosition=1;
                ShortMargin(i)=0;   %平空后空头保证金为0了
                MyEntryPrice(i)=UpperLine(i-1)+ImpactCost;
                if Open(i)>MyEntryPrice(i)    %考虑是否跳空
                    MyEntryPrice(i)=Open(i)+ImpactCost;
                end
            ...
            %空仓开多
            if MarketPosition==0 && BuyCount==n3
                MarketPosition=1;
                MyEntryPrice(i)=UpperLine(i-1)+ImpactCost;
                if Open(i)>MyEntryPrice(i)    %考虑是否跳空
                    MyEntryPrice(i)=Open(i)+ImpactCost;
    ...
    if MarketPosition~=-1 && SumUd(i)==n2
        ...
        for j=i+1:length(data)
            ...
            if MarketPosition==1 && SellCount==n3
                MarketPosition=-1;
                LongMargin(i)=0;     %平多后多头保证金为0了
                MyEntryPrice(i)=LowerLine(i-1)+ImpactCost;
            ...
            %空仓开空
            if MarketPosition==0 && SellCount==n3
                MarketPosition=-1;
                MyEntryPrice(i)=LowerLine(i-1)+ImpactCost;
                if Open(i)<MyEntryPrice(i)
                    MyEntryPrice(i)=Open(i)+ImpactCost;
        ...
        if i==length(data)
            ...
            if MarketPosition==1
                ...
                LongMargin(i)=0;
                ClosePosNum=ClosePosNum+1;
                ClosePosPrice(ClosePosNum)=Close(i)+ImpactCost;%记录平仓价格
                CloseDate(ClosePosNum)=Date(i);%记录平仓时间
                StaticEquity(i)=StaticEquity(i-1)+(ClosePosPrice(ClosePosNum)-OpenPosPrice(OpenPosNum))...
                    *TradingUnits*Lots-OpenPosPrice(OpenPosNum)*TradingUnits*Lots*TradingCost...
                    -ClosePosPrice(ClosePosNum)*TradingUnits*Lots*TradingCost;
            ...
            if MarketPosition==-1
                ...
                ShortMargin(i)=0;
                ClosePosNum=ClosePosNum+1;
                ClosePosPrice(ClosePosNum)=Close(i)+ImpactCost;
